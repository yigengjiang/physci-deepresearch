"""Standalone OpenAI judge client.

Vendored from `core/utils/openai_utils/simplified_client.py` (origin/submit).
Imports only `openai` + the local `._openai_types` — leak-free.

The class body is copied VERBATIM. The ONLY edits are:
  (i) the relative import `from .types import ...` -> `from ._openai_types import ...`;
  (ii) `JudgeClient` is exposed as the public name (an alias of the ported
       `SimplifiedAsyncOpenAI`), still subclassing `openai.AsyncOpenAI` so that
       `beta.chat.completions.parse(response_format=<pydantic>)` is inherited.

Env-var fallbacks inside the client remain the source `UTU_LLM_*` reads
verbatim; the canonical `JUDGE_LLM_*` precedence is applied upstream in
`eval.config`, which passes explicit api_key/base_url/type/model values.
"""
...
# Public alias: the released scorer constructs `JudgeClient(**provider_kwargs)`.
# Identical class; subclasses `openai.AsyncOpenAI` so `beta.chat.completions.parse`
# (used by the long-form structured-output judge path) is inherited unchanged.
JudgeClient = SimplifiedAsyncOpenAI
